...
function ServiceIcon({ item }: { item: ServiceItem }) {
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.6}
      onPress={() => safeRouter.replace(`/app/afupay?section=${item.route.split("/").pop()}` as any)}
    >
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.backgroundSecondary }]}>
      <GlassHeader
        ...
        right={
          <TouchableOpacity hitSlop={12} onPress={() => safeRouter.replace({ pathname: "/app/afupay", params: { section: "fee-details", service: "airtime", amount: "100", fee: "2", total: "102" } } as any)}>
      ...
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 32 }]} showsVerticalScrollIndicator={false}>
        <View style={[styles.topBanner, { backgroundColor: colors.accent }]}>
          <TouchableOpacity style={styles.topBtn} activeOpacity={0.7} onPress={() => safeRouter.replace("/app/afupay?section=transfer" as any)}>
