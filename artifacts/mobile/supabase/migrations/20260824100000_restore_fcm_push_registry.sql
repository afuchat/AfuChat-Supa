-- Recreate the direct-FCM registry after the legacy push cleanup migration.
...
CREATE POLICY "push_devices_insert_own" ON public.push_devices FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "push_devices_update_own" ON public.push_devices;
CREATE POLICY "push_devices_update_own" ON public.push_devices FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "push_devices_delete_own" ON public.push_devices;
CREATE POLICY "push_devices_delete_own" ON public.push_devices FOR DELETE USING (auth.uid() = user_id);